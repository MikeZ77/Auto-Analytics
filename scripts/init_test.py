        ...
        def setupLinks(parse):
            ...
            for tag in parse:
                if 'href' in tag.a.attrs:
                    links.append('https://www.autotrader.ca' + tag.a.attrs['href'])
            return links
        # LINKS PAGE
        # setup connections
        cls._req = a.requests.get(TEST_LINK, headers=HEADER)
        cls._dbConn = a.mysql.connector.connect(user=os.environ['USER_NAME'], passwd=os.environ['PASSWORD'], host=os.environ['HOST_NAME'], database=os.environ['DATABASE'])
        # setup page objects
